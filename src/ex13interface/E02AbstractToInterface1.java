...
추상메서드를 포함하고 있으므로 하위에서 상속받게되면 받드시 추상메서드를
오버라이딩 해아한다.
 */

abstract class PersinalNumberStorageAbs{
	//정보를 젖장하는 기능의 추상메서
	public abstract void addPersonalInfo(String juminnum, String name);
	//저장된 정보를 검색하는 기능의 추상메서드
	public abstract String searchPersonalInfo(String juminMum);
	
}

/*
DTO(Data Transfer Objerct) : 데이터를 저장하고 전송하기 위한 용도의
	객체를 의마한다. VO(Value Object)로 표현하기도 한다. 프로그램에서
...
기능을 구현한 클래스
 */
class PersonalNumberStorageExt extends PersinalNumberStorageAbs{
	...
	해야한다. 이는 필수사항이며 지키지 않으면 에러가 발생한다.
	 */
	@Override

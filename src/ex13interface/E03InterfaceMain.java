...
/*
인터페이스(interface)
-클래스가 각체의 설계도라면 인터페이스는 클래스의 설계도라 할수있다.
-자바는 단일 상속을 원칙으로 하지만 인테페이스를 통해 다중 상속을 구현
할 수 있다.
-추상클래스와 동일하게 상속을 목적으로 제작되므로 구현받은 하위클래스
에서는 반드시 추상메서드를 오버라이딩 해야한다.
-멤버로는 추상메서드와 상수만 선언할수 있다.
	메서드 : public abstract
...
//	public MyInterface1() {}
	...
	/*
	멤버상수 : 인터페이스에 선언되는 변수는 상수로 선언한다. 따라서
		대문자로 작성해야 한다.
	 */
...
public class E03InterfaceMain {
	...
	public static void main(String[] args) {
		/*
		'구현'한 인터페이스를 통해 참조변수를 선언할 수 있다. MyClass
		의 부모는 모두 3개이다.
		 */
		...
		//MyClass의 부모는 모두 3개이므로 아해는 모두 참조 가능하다.

...
추상메서드
-오버라이딩을 목적으로 제작되었더.
-메서드의 실행부가 없을므로 중괄호 대신 세미콜론(;)으로 마무리한다
 */
//추상클래스1 : 추상메서드들 포함하지 않은 클래스
...
인스턴스를 생성할 수 없게된다. 따라서 인스턴스 생성을 위해서는 부모으ㅟ
...
class AbsClassChild2 extends ABsClass2 {
	...
	따라서 인스턴스 생성이 가능해진다.
	현재 오버라이딩 한 메서드룰 삭제하면 에러가 발생한다.
	 */
	void instanceMethod() {
		System.out.println("선택적 오버라이딩");
	}
	void absMethod(int number) {
		System.out.println("필수 오버라이등");
	}
	void newMethod() {
...
//		AbsClass1 absClass1 = new AbsClass1();
		...
		//추상클래스를 상속한 하위클래스는 인스턴스를 생성을 할 수 있다.
		...
		/*
		추상클래스내에 선언돤 정적메서드는 오버라딩의 대상이 될 수 

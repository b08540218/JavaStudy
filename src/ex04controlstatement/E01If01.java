...
//		}
		...
		//if문의 조건식은 아래와 같이 boolean값을 반환해야한다.
		//비교식으로 처리
		if(num%2==0 && num>=10) {
		...
		/*
		 ...
		 상관없이 중괄호에 있는문장이 싱행된다. 딸라서 사용에 
		 주의해야한다.
		 */
		if(num%2 != 0);
			System.out.printf("%d는 홀수입니다.", num);
			/*
			if문의 실행문장이 1개인 경우 중괄호를 생략할수 있다.
			단, 실행문이 2개이상이람녀 첫번째 문장만 if문에 포함되므로
			반드시 중괄호를 기술해야한다.
			 */
		if(num%2 == 0)
			System.out.println("\n"+ num +"은 짝수이다");
		...
		if(num%2 != 0)
			...
			int asciiCord = System.in.read();
			//a는 97, 1은 49가 출력됨
			System.out.println("입력한 문자는:"+ asciiCord);
			
			/*
			아스키코드를 알고있을때의 구현방법. 문자 0과 9는 아스키코드
			48, 57에 해당하므로 구간내에 있는지 확인하면 된다.
			*/
			if(asciiCord>=48 && asciiCord<=57) {

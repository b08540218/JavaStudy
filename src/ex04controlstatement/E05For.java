...
public class E05For {
	...
	public static void main(String[] args) {
		...
		for(;;)	{
			...
			탈출하지 못하면 아래에 있는 코드는 실행될 수 없는 코드
			되므로 "Unreachable code"라는 에러가 발생하게 된다.
			다랄서 아래와 같이 탈출조건을 만들어 줘야한다.
			 */
			if(a==10) break;
		}
		/*
		for문 끝에 ;을 사용하면 반복할 문장이 없는것으로 간주되어
		for문과 상관없이 한번만 실행된다.
		 */
		...
		/*
		for문의 초기식에서 선언된 변수j는 for문이 종료되는 순간
		메모리에서 소멸된다. 여기서 사용된 변수j는 지역변수(Local varuable)
		라고 한다. 지역변수는 해당 지역을 벗어나면 즉시 소멸되는 특징을 가지고있다.
		 */

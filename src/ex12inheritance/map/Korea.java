...
public class Korea {
	...
	public void publiMethod(String loc) {
		/*
		public으로 선언된 멤버메서드는 어디서든 접근이 가능하다.
		동일패키지, 다른패키지, 외부클래스, 상속 등 모든 영역에서
		접근할 수 있다.
		 */
		System.out.println(loc +"publiMethod() 호출");
		/*
		default로 선언된 메서드는 패키지가 다른 클래스에서는
		호출할 수 없으므로 해당 메서드를 통해 간접 호출해야한다.
		 */
		defaultMethod("publiMethod->");
	}
	public void protectedMethod(String loc) {
		/*
		서로 패키지가 다른 2개의 클래스가 상속관계를 가지고 있다면
		접근을 허용한다. 즉 default 접근지정자보다는 넓은 접근법위를
		가진다.
		 */

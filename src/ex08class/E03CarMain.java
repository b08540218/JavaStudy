...
/*
이 예제에서는 Huamn클래스를 별도의 파일로 작성하였다. 동일한 패키지에
	정의되어있다면 컴파일러는 해당 클래스를 찾아 즉시 인스턴스로 만들수 있다.
	만약 다른 패키지에 선언된 클래스라면 import 선언을 해아한다.
 */
class Car{
	//멤버변수 : 모델명, 소유주
	String carModel;
	...
	소유주를 표현한 멤버변수는 Human 타입으로 정의한다. 우리가 직접
		작성한 클래스는 객체를 표현하기 위해 생성했지만, Java에서는
		하나의 자료형으로 사용된다. 즉 객체를 추상화 한 또 하나의 자료형을
		생성한 것이 class를 만드는 목적이라 할수있다.
	 */
	...
	/*
	초기화 메서드2 : 위와 동일한 이름의 메서드로 오버로딩에 의해
...
public class E03CarMain {
	...
	public static void main(String[] args) {
		...
		/*
		자동차객체1 생성 : 매개변수가 없는 초기화 메서드를 호출하므로
			'람보르기니'로 초기화 된다. */
		Car car1 = new Car();
		car1.initialize();
		car1.showCarInfo();
		/*
		자동차객체2 생성 : 초기화 메서드를 호출하는 대신 맴버변수에
			직접 접근하여 인스턴스를 초기화한다. 이 경우 인스턴스를
			생성할때마다 동일한 코드를 작성해야 하므로 비효율적이다.
			따라서 이 방법은 사용하지 않는 것이 좋다.
		 */
		Car car2 = new Car();
		car2.carModel = "밴틀리";
		...
		car2.showCarInfo();
		/*
		자동차객체3 생성 : 매개변수가 있는 초기화 메서드를 사용해서
			인스턴스를 초기화 한다. 이 경우 매개변수를 통해 다양한 값을
			전달할 수 있으므로 가장 효율적인 코드라 할 수 있다.
		 */
		Car car3 = new Car();
		car3.initialize("스포츠카", "성유겸", 8, 10);
		car3.showCarInfo();
		/*
		자동차객체4 생성인스턴스 생성만 하고 초기화하지 않은 상태로 출력을 시도하므로
			예외가 발생한다.
		 */
		Car car4 = new Car();
		car4.showCarInfo();

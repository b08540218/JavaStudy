...
class FruitSeller{
	...
	초기화 할수 있다.
	하지만 멤버상수는 인스턴스 생서오가 상관없이 저으이하는 시점에 받드시
	초기화해야한다. 즉 100을 채우면 에러가 발행하게된다.
	 */
	//과일 판매를 위한 멤버메소드
	/*
	1.구매자로부터 금액을 전달 받는다.(메서드 호출시 매개변수를 통해 받음
	2.금액/단가=판매될 사과의 갯수를 계산하낟.
	...
	public void showSaleResult() {
		...
		System.out.println("[판매자]남은수익:"+ numOfApple);
	}
}
//구매자
class FruitBuyer{
	...
	1.판매자에게 금액을 지불한다. => 여기에서 seller인스턴스의 메서드를
		호출한다.
	2.판매자가 금액에 해당하는 사과의 갯수를 반환하면, 그만큼
		사곽가 증가한다.
		=> numOfApple += 메서드호출()
...
public class E06FruitSalesMain1 {
	...
	public static void main(String[] args) {
		...
		System.out.println("구매행위가 일어낙전의 상태");
		//재고수량 100개

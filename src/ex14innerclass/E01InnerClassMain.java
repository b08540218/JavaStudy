...
-외부 클래스의 모든 멤버(정적 혹은 인스턴스형)을 사용할 수 있다.
-정적 멤버를 가질 수 없다. 단 상수는 가질 수 있다.
-컴파일시 외부클래스명$내부클래명.class 파일이 생성된다.
...
class Unit{
	...
	public Unit(String name, int healthPoint) {
		...
		//인스턴스 맴버변수는 new를 통해 생성
		marine = new Marine();
		//Medic 인스턴스는 생성은 주석으로 처리하고 외부에서 생성
//		medic = new Medic();
	}
	//멤버변수의 정보를 문자열로 반환
	...
	//내부클래스 정의(내부 멤버 클래스)
	class Marine{
		//내부클래스도 일반적인 클래스와 동일하게 멥버를 정의 할수 있다
		int attackPower;
		public void attack() {
			/*

...
public class FriendInfoHandler {
	...
	public FriendInfoHandler(int num) {
		/*
		핸들러 클래스의 인스턴스를 생성할때 데이터 저장을 위한Friend
...
//		System.out.println("addFriend 호출됨");
		...
		Friend 베열에 저장한다. 즉 상속을 통해 타입이 다른 자식
		...
		즉 High, Univ 인스턴스는 Friend 타입의 인스턴스로 
		자동현변환되어 저장된다.
		 */
	...
	public void showAllData() {
		...
		for (int i = 0; i < numOfFriends; i++) {
			 ...
			 */
			//저장된 인스턴스가 원래 어떤 타입이였는지 확인 후 형변환
			if (myFriends[i] instanceof UnivFriend) {
				((UnivFriend)myFriends[i]).showAllDate();
			} 
	...
	public void showSimpleData() {
		...
		for (int i = 0; i < numOfFriends; i++) {
			if (myFriends[i] instanceof UnivFriend) {
				((UnivFriend)myFriends[i]).shswBasicInfo();;
			} 
			else if (myFriends[i] instanceof HighFriend){

...
/*
주소관리프로그램에서 기능을 담당하는 핸들러 클래스 정의.
...
//		Scanner scan = new Scanner(System.in);
		...
		if (choice==1) {
			...
			*/
			...
			//대딩인 경우 전공을 추가로 입력
			System.out.println("전공:");iMajor = MyFriendInfoBook.scan.nextLine();
	...
	public void showAllData() {
		/*고딩친구반복 : 저장된 정보의 갯수만큼 반복해서 전체정보를
	...
	public void showSimpleData() {
		...
		//대딩친구반복 
		for (int i = 0; i < numOfunivFriend; i++) {
			univFriends[i].shswBasicInfo();
		}
		System.out.println("##간략전보가 출력되었습니다.##");
	}
	...
	public void deleteInfo() {
		...
		*/
		...
		int deleteIndexUniv = -1;
			...
			//고딩친구반복
			for (int i = 0; i < numOfhighFriend; i++) {
				/*삭제할 이름과 일치하는 인스턴스를 찾기위해 equals()
			...
			if(deleteIndexHigh!=-1) {
				...
				for (int i=deleteIndexHigh; i < numOfhighFriend; i++) {
					/*
					삭제된 인스턴스의 인덱스보다 마지막까지 반복하면서 뒤쪽의
				...
				//고딩/대딩모두 -1을 유지한다면 검색된 값이 없다는 의미.

...
public class Ex05Finally {
	...
	static void tryCatchFinally() {
		...
		int age = -1;
		try {
			//'10살'과 같이 입력하면 예외가 발생됨
			age = scan.nextInt();
			System.out.println("당신은 5년후 "+ (age+5)+ "살 입니다");
			/*
			앞에서 정삭적인 값을 입력하더라도 return이 실행된다.
			...
			터미네이트 버튼을 누르른 것과 동일하게 동작한다.
			따라서 try절에서 해당 명령을 만나게 되면 fianlly절은
			실행되지 않는다.
			 */
			System.exit(0); //주석 처리시 79열 "메인메소드 끝" 출력
		}
	}

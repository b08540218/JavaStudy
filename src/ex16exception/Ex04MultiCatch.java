...
이 경우 반드시 자식 예외 클래스부터 catch블럭에 추가해야 한다.
...
public class Ex04MultiCatch {
	...
	public static void main(String[] args) {
		...
		try {
			...
			/*
			1번 인덱스에 0을 입력하면 연산이 불가능 하므로 예외가 발생
			된다.
			 */
			System.out.println("배열에 저정된 두 숫자나누기:" + array[1]/array[1]);
			
			//나이 입력시 문자를 입력하면 예외가 발생된다.
			System.out.print("나이를 입력하세요:");
			int age = scanner.nextInt();
			System.out.println("당식의 나이는:"+ age);
			/*
			...
			/*
			Exception클래스는 모든 예외클래스의 부모이므로
			catch절의 마지막에 위치해야 한다. 앞부분에 있으면
		...
		/*
		앞 부분에서 예외가 발생하더라도 예외처리를 했으므로 프로그램

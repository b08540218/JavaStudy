...
-메서드 호출 후 반환값은 호출한 위치로 반환된다. 이때 메소드는 메모리에서
	소멸된다.
-반환값이 없는 void형 메서드로 실행이 완료되면 호출한 위치로 실행의
	흐름이 돌아온다.
 */
...
public class E01MethodBasic {
	/*
	메서드1 : 반환값O, 매개변수O
	=> 매개변수로 2개의 정수를 전달받아 합의 결과로 반환된다.
	반환값은 호출한 지점으로 반환된다.
	 */
	...
	/*
	메서드2 : 반환값X, 매개변수X
	=> 전달받는 값 없이 이름을 입력받은 후 namePirnt() 메서드를
	호출한다. 이와같이 반환값이 없는 경우에는 반드시 void를 명시해야
	한다. 
	*/
	public static void menuPrint() 
	{
		System.out.println("당신의 이름은 무엇인가?");
	...
	/*
	메서드3 : 반환값X, 매개변수O
	=> 문자열을 인수로 잔달받아 단순히 출력한다. 반환값이 없는 경우

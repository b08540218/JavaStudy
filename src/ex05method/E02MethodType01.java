...
-메서드는 반드시 class내부에 정의해야한다.
-메서드 내부에 또 다른 메서드를 정의할 수 없다. 이 경우 에러가
	발생한다.
...
-상수명 : SIMPLE_CONST
	=> 전체를 대문자로 기술. 단 연결되는 단어는 _로 구분
-패키지명 : simple.package
	=> 전체를 소문자로 기술. 열결되는 단어는 .으로 구분
 */
...
public class E02MethodType01 {
	...
	static void menuPrint() {
		...
		System.out.println("1.열기, 2.계속열기, 3.종료");
		System.out.println("=================");
	}
	...
	static void returnError() {
		...
		그 아래 코드는 실행될 수 업슨ㄴ 상태가 된다. 따라서 에러가
		발생된다.
		이런 경우에는 반드시 조건문과 같이 기술해서 조건부로 실행될 수 
		있도록 코딩해야한다.
		단 메서드의 마지막 부분에 기술 하는 경우에는 조건문이 없어도 된다.
		 */

...
'복사'를 통해 값이 전달된다.
그래서 다른 지역의 메서드에서 값의 변경이 있더라고 호출한 메서드의 값에서는
전혀 영향을 미치지 않는다.
...
public class E03CallByValue {
	...
	public static void main(String[] args) {
		...
		int first=100, second=200;
			...
			/*
			매개변수로 전달된 값은 복사되어 다른 지역의 메서드 내에서만
			변경되므로 main메서드에서는 초기값이 그대로 유지된다.
			 */
	...
	public static void callByValue(int fNum, int sNum) {
		/*
		Swap(스왑) : 두개의 뱐수가 할당받은 값을 서로 교환하는 것을
		...
		//해당 메서드가 종료되면 호출한 지점으로 돌아간다. callByValue(first, second); 23행
	}

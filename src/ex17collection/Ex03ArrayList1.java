...
//		Vector<String> list = new Vector<String>();
		...
		인스턴저장 : add()
			add(인스턴스) : 순차적으로 저장하면서 인덱스는 0부터
				자동으로 부여된다.
			add(인덱스, 인스턴스) : 데이터 저장시 인뎃스를 직접 
				부여한다. 단, 인덱스를 건너뛰면 에러가 발생되므로
				주의해야 한다. 저장하려는 인덱스에 이미 저장된
...
//		list.add(5, "소방챀ㅋㅋ"); // 에러발생됨
		...
		/*
		전체삭제 : 아래 2개의 메서드를 통해 컬랙션에 저장된 전체
			데이터를 삭제할 수 있다.
		 */
		list.removeAll(list);
		list.clear();

...
public class Ex04HashSet1 {
	...
	public static void main(String[] args) {
		...
		생성한다.
		 */
		String varSting1 = "JAVA";
		String varSting2 = new String("개발자");
		...
		인스턴스 저장 : add()메서드를 통해 저장한다. 저장에 성공한
			경우 true를 반환한다.
		 */
		System.out.println("add결과:"+set.add(varSting1));
		set.add(varSting2);
		set.add(varDate);
		set.add(varInt);
		set.add(varTeacher);
		...
		/*
		기본클래스의 객체 중복저장
		: 기본 클래스의 인스턴스의 경우 별도의 오버라이딩 없이도 중복이 
		제거된다. 따라서 아래의 add()는 false를 반환한다.
		 */
		System.out.println(set.add(varSting2)? "저장성송" : "저장실패");
		//저장에 실패했으므로 5개 출력됨
		...
		포함여부 확인 : set에 해당 인스턴스가 저장되어 있는지 확인하여
			boolean을 반환한다.
		 */
		System.out.println(set.contains(varSting1)? "varString1 있음": "varString1 없음");
		
		/*
		인스턴스 삭제 : set은 인덱스를 사용하지 않으므로 인스턴스의
			참조값을 통해 삭제한다.
		 */
		System.out.println(set.remove(varSting2)? "varString2 삭제 성공": "varString2 삭제 실패");

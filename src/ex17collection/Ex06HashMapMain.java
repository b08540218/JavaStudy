...
-Key값으로 검색되므로 다른 컬렉션에 비해 속도가 빠른다.
-저장 순서는 유지되지 않는다
 */
public class Ex06HashMapMain {
	...
	public static void main(String[] args) {
		...
		//int는 String으로 혛변환 할 수 없으므로 타입에러가 발생된다.
		int number = 20;
//		map.put("age", number);
		...
		/*
		중복저장 : 기존에 입력된 name이라는 Key가 있기 때문에
		...
		/*
	 	...
	 	: keySet()을 통해 전체 ket를 Set으로 얻어온다
		...
		/*
		객체의 존재유무 확인
		: Map은 Key와 value가 있으므로 메서드도 2개로 나눠져
		있다.
		 */
		...
		객체삭제 : Key를 통해 삭제하고, 삭제에 성공하는 해당
			value가 반환된다.
		 */
		System.out.println("삭제될 객체:"+ map.remove("age"));

...
public class Ex07AsList {
	...
	public static void listPrint(String title, List<String> list) {
		...
		/*
		출력할 인스턴스가 String이므로 별도의 오버라이딩 없이고ㅓ
	...
	public static void main(String[] args) {
		/*
		String 타입의 배열을 생성과 동시에 초기화. 이 경우 
		초기데이터의 갯수를 통해 크기를 유추할 수 있으므로, 크기가
		4인 배열이 생성된다.
		 */
		String[] strArr = new String[]{"마린", " 파이어뱃", " 메딕", " 마린"};
		/*
		Arrays.asList() : 일반 배열을 List<E>로 변환해주는
			함수. 단 값을 참조만 할 수 있거, 입력이나 삭제는
			할 수 없다.
		 */
		List<String> list =Arrays.asList(strArr);
		listPrint("출력1", list);
		
		//현재 상태에서는 데이터 추가가 불가능 하므로 런타임에러가 발생된다.
//		list.add("탱크"); //에러발생
		
		/*
		ArrayList<E>의 생성자를 통해 인스턴슬 복사하면 데이터
		변경을 할 수 있다.
		 */

...
public class E05Date {
	...
	public static String myDateFormat(Calendar c, String formatStr) {
		/*
		서버의 표준시인 1970-01-01 부터 현재까지의 시간을 초 단위
		로 반환해주는 메서드로 이를 '타임스템프'라고 한다.
		 */
		Date date = c.getTime();
	...
	public static void main(String[] args) {
		...
		/*
		월을 표현하는 Month, 분을 표현하는 Minute의 첫글자가
		...
		/*
		앞에서 add를 3번 호출했으므로 결과는 아래와 같이 
		1년 2개월 3일후의 날짜가 출력된다.
		 */
	...
	public static long myDiffDate(String today, String future) {
		...
		try {
			...
			따라서 각 날짜의 서식을 명확히 지정해야한 날짜를 인식하여 연산
			할 수 있다.
			 */
			//각 서식의 날짜를 파싱하여 Date 인스턴스로 변환한다.
			Date formatToday = new SimpleDateFormat("yyyy-MM-dd").parse(today);
			Date formatFuture = new SimpleDateFormat("yyy/MM/dd").parse(future);
			System.out.println(formatToday+" <> "+formatFuture);
			
			/*
			getTime()으로 얻어오는 시간은 13자이의 정수값이다.
			10자리까지는 순수한 초(Sec)이고, 11~13자리는 밀리세컨즈를 
			의미한다. 즉 100으로 나눈 후 뒤의 3자리를 제거하면 순수한 초가
			반환된다. 또한 하루는 60*60*24 즉 86400초 이므로 아래와
			같이 남은 날짜를 계산할 수 있다.
			 */

...
//		System.out.println("120원:"+ Integer.parseInt(money)); //예외 발생
		...
		/*
		실수형태의 문자열을 숫자로 변경할때 parseInt()는 사용할 수
		없다. 앞과 동일한 예외가 발생한다.
		 */
		String floatString = "3.14";
//		System.out.println("120원:"+ Integer.parseInt(floatString));
		...
		codePointAt() : 문자열에서 특정 index에 해당하는 한 문자의
			아스키코드를 반환한다.
		 */
		System.out.println("ABC에서 3번째 인덱스 D의 아스키코드");
		...
		isWhitespace() : 공백문자인지 판단하는 메서드. 공백문자일때
			true를 반환한다.
		 */
		System.out.println("isnWhitespace()메소드로 공백문자 판단");
		...
		isUpperCase() : 대문자인지 판단. 단 영문이 아닌경우
			false를 반환한다
		 */
		...
		판단하는 프로그램을 작성하시오.
			참고) toCharArray() : 문자열을 char형 배열로 
			반환해주는 String클래스에 정의된 메소드. 
		*/
		...
		System.out.println("총공백수:"+ whitCnt);
}
}

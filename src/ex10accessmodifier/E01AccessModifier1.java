...
/*
해당 클래스는 접근지정자는 생략했으므로 default 클래스로 정의된다.
이 경우 동일한 패키지 내에서만 접근을 허용한다.
 */
class DefaultClass1 {
	void myFunc() {
		System.out.println("DefaultClass클래스의 myfFunc()메소드 호출");
	}
}

/*
public 클래스는 해당 Java파일을 대표하는 클래스로 파일명과 동일해야한다.
또한 public 클래스는 자바파일당 단 하나만 정의할수 있다.

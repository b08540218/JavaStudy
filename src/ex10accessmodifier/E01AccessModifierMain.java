...
/*
해당 클래스는 sub패키지에서 작성되었으므로 사용을 위해서는 반드시 
import 해야한다.
 */
...
public class E01AccessModifierMain {
	...
	public static void main(String[] args) {
		...
		접근 할 수없다. 유일하게 동일한 클래스 내부에서만 접근을
		허용한다.
		 */
//		System.out.println("one.privateVar="+ one.privateVar);// 에러발생
		System.out.println("one.defaultVar="+ one.defaultVar);
		System.out.println("one.publicVar="+ one.publucVar);
		
		//멤버메서드로 변수와 동일한 접근 법위를 가진다.
//		one.privatVarMethod();// 에러발생
		/*
		private으로 선언된 멤버에 외부클래스에서 접근하려면 default
...
//		System.out.println("two.privateVar="+ one.privateVar);// 에러발생
//		System.out.println("two.defaultVar="+ one.defaultVar);// 에러발생
		System.out.println("two.publicVar="+ one.publucVar);
		
//		two.privateMerhod();// 에러발생
//		two.defaultMethod();// 에러발생
		two.publicMethod();
		
		/*
		다른 패키지에 선언된 디폴트 클래스이므로 인스턴스 생성이
		불가능하다. import 자체가 불가능 하므로 아예 사용할 수 업ㅆ는
		클래스가 된다.
		 */
		System.out.println("DefaultClass 객체 생성 및 접근");
